...
Build the command and run it in the background with Monitor so you can react to live output:
...
Use Bash with `run_in_background: true` and timeout **300 000 ms** (5 min — model load takes up to 90 s on cold start). Immediately follow with a Monitor tool call on the same process to stream each output line as a notification.

---

## Step 5 — Output Monitoring

Watch the live Monitor stream and react to each line:
...
| `[AUTH REQUIRED]` | Tell the user: *"Fara is paused at a login wall. Switch to the Edge window and log in. When done, press Enter in the terminal where fara is running (Claude cannot do this for you)."* Continue monitoring — fara resumes automatically after Enter |
